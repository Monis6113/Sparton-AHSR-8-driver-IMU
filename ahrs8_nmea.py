...
def verify_checksum(response):
    ...
    for character in message:
        ...
        calculated_checksum = calculated_checksum ^ ord(chr(character))
    ...
    print('hexstr: ', hexstring.encode(), "            checksum: ", checksum)
    return hexstring.encode() == checksum

# Populate message angular velocity values.
def populate_G(string, message):
    split_string = string.decode().split(",")
